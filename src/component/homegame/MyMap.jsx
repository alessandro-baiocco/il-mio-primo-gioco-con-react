...
const MyMap = (props) => {
  ...
  let generateLevels = Array.from(Array(props.stages[playerStages].levels).keys());
  ...
  return (
    <Container className="d-flex" style={{ position: "relative" }}>
      ...
      {generateLevels.map((level, i) => (
        ...
        >
          {`level ${i + 1}`}
          {props.stages[playerStages].fight.includes(i) && (
          ...
          <img
            ...
            className={`${i === playerPosition ? "" : "d-none"} img-fluid`}
            alt="player-position"
